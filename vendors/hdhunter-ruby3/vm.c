...
void init_shared_mem(void) {
    if (afl_area_ptr == NULL && getenv(SHM_ENV_VAR) != NULL) {
    ...
    if (execution_path == NULL && getenv("__EXECUTION_PATH") != NULL) {
        ...
        printf("Using %d to attach to execution_path\n", exec_path_id);
        ...
        printf("exec_path_id=%d resulted in address %p \n", exec_path_id, execution_path);
    }
    ...
    { \
        ...
        if (afl_area_ptr != NULL) { \
            if (hdhunter_trace != NULL) { \
                fprintf(hdhunter_trace, "%010lu\n", ec->cfp->iseq->body->iseq_unique_id); \
