...
//     if (line) {
// 	fprintf(stderr, "iseq: %lx %lx %p %s \n", ec->cfp->iseq->body->iseq_unique_id, ec->cfp->iseq->body->iseq_encoded, ec->cfp->iseq->body->iseq_encoded, posbuf);
//     } else {
//         fprintf(stderr, "iseq:  %lx %lx %p \n", ec->cfp->iseq->body->iseq_unique_id, ec->cfp->iseq->body->iseq_encoded, ec->cfp->iseq->body->iseq_encoded);
//     }
...
vm_exec(rb_execution_context_t *ec, int mjit_enable_p)
{
    ...
    EC_PUSH_TAG(ec);
    //fprintf(stderr, "I'm into in vm_exec top\n");
    //witcher_trace(ec);
    //control_frame_dump(ec, ec->cfp);
    WITCHER_TRACE();
	...
	result = ec->errinfo;
        ...
        while ((result = vm_exec_handle_exception(ec, state, result, &initial)) == Qundef) {
            ...
            result = vm_exec_core(ec, initial);
	  vm_loop_start:
            //fprintf(stderr, "\t in vm_exec: vm_loop_start %p\n", ec->cfp->pc);
            //fprintf(stderr, "\tinsn: %"PRIdVALUE, GET_CURRENT_INSN());
