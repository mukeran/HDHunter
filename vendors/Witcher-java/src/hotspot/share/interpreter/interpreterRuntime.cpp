...
void load_include_list(){
    ...
    char *filter_path = getenv("HDHUNTER_WITCHER_JAVA_FILTER_PATH");
...
void cgi_get_shm_mem() {
    ...
    // if (afl_info){
        ...
        if (afl_area_ptr == NULL){
            int afl_id = atoi(getenv("__AFL_SHM"));
...
int method_name_hash(char *meth_sig, int length){
    int hash = 5381;
    int c;
    while ((c = *meth_sig++) && length--){
...
//        }
    ...
    if (afl_area_ptr != NULL /*&& afl_info->capture*/) {
        ...
        // if (thread_index >= 0){
            char* meth_sig = (char*)malloc(110);
            memset(meth_sig, 0, 110);
            last_frame.method()->name_and_sig_as_C_string(meth_sig, 100);
...
//                           last_frame.bytecode().java_code(), thread_items[thread_index].instruction_kicker, thread_items[thread_index].prior_value);
                ...
                if (isEdge(last_frame.bytecode().java_code())/* || thread_items[thread_index].prior_value == 0*/){
                    ...
                    // instruction_order++;
                    // int method_index = last_frame.method()->name_index();
                    int method_index = method_name_hash(meth_sig, 100);
                    int bci = last_frame.bci();
                    // Cantor pairing function

                    uint cur_edge_cantor_value = (method_index + bci) * (method_index + bci + 1) / 2 + method_index;
                    ...
                    uint idx = cur_edge_cantor_value % MAPSIZE;
                    afl_area_ptr[idx]++;
                    if (!visited_edges[idx]){
                        visited_edges[idx] = true;
                        *execution_path = *execution_path * 31 + idx;
                    }
                    if (hdhunter_trace != NULL) {
                        fprintf(hdhunter_trace, "%010u\n", cur_edge_cantor_value);
