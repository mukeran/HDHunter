...
use super::*;
use fuzz_runner::nyx::aux_buffer::{NYX_ABORT, NYX_CRASH, NYX_HPRINTF};
...
#[no_mangle]
pub extern "C" fn nyx_config_set_input_buffer_write_protection(
    config: *mut c_void,
...
#[no_mangle]
pub extern "C" fn nyx_config_set_reuse_snapshot_path(
    config: *mut c_void,
    reuse_snapshot_path: *const c_char,
) {
...
#[no_mangle]
pub extern "C" fn nyx_config_set_aux_buffer_size(
    config: *mut c_void,
    aux_buffer_size: u32,
) -> bool {
    let cfg = __nyx_config_check_ptr(config);
...
pub extern "C" fn nyx_get_input_buffer(nyx_process: *mut NyxProcess) -> *mut u8 {
    unsafe {
        return (*__nyx_process_check_ptr(nyx_process))
            .input_buffer_mut()
            .as_mut_ptr();
    }
}
...
#[no_mangle]
pub extern "C" fn nyx_option_set_timeout(
    nyx_process: *mut NyxProcess,
    timeout_sec: u8,
    timeout_usec: u32,
) {
    unsafe {
...
pub extern "C" fn nyx_exec(nyx_process: *mut NyxProcess) -> NyxReturnValue {
    unsafe { (*__nyx_process_check_ptr(nyx_process)).exec() }
}
...
pub extern "C" fn nyx_print_aux_buffer(nyx_process: *mut NyxProcess) {
    unsafe {
        ...
        print!(
            "{}",
            format!("{:#?}", (*nyx_process).process.aux_buffer().result)
        );
        ...
        match (*nyx_process).process.aux_buffer().result.exec_result_code {
            NYX_CRASH | NYX_ABORT | NYX_HPRINTF => {
                println!(
                    "{}",
                    std::str::from_utf8(&(*nyx_process).process.aux_buffer().misc.data).unwrap()
                );
            }
            _ => {}
        }
...
#[no_mangle]
pub extern "C" fn nyx_get_aux_string(
    nyx_process: *mut NyxProcess,
    buffer: *mut u8,
    size: u32,
) -> u32 {
    unsafe {
        ...
        let len = std::cmp::min(
            (*nyx_process).process.aux_buffer().misc.len as usize,
            size as usize,
        );
        std::ptr::copy(
            (*nyx_process)
...
pub extern "C" fn nyx_set_hprintf_fd(nyx_process: *mut NyxProcess, fd: i32) {
    unsafe {
        (*__nyx_process_check_ptr(nyx_process))
            .process
            .set_hprintf_fd(fd);
    }
}
