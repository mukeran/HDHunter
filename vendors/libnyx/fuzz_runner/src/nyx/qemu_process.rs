...
impl QemuProcess {
    pub fn new(params: QemuParams) -> Result<QemuProcess, String> {
        ...
        shm_work_dir_path.push("http_param");

        let http_param_shm_f = OpenOptions::new()
            .create(true)
            .read(true)
            .write(true)
            .open(&shm_work_dir_path)
        ...
        if Path::new(&format!("{}/http_param_{}", params.workdir, params.qemu_id)).exists() {
            fs::remove_file(format!("{}/http_param_{}", params.workdir, params.qemu_id)).unwrap();
        }

        symlink(
            &shm_work_dir_path,
            format!("{}/http_param_{}", params.workdir, params.qemu_id),
        )
        .unwrap();
        ...
        http_param_shm_f.set_len(0x1000).unwrap();
        execution_path_shm_f.set_len(0x1000).unwrap();
        ijon_buffer_shm_f.set_len(0x1000).unwrap();

        let mut bitmap_shared = make_shared_data(&bitmap_shm_f, params.bitmap_size);
        let http_param_shared = make_shared_data(&http_param_shm_f, 0x1000);
        let execution_path_shared = make_shared_data(&execution_path_shm_f, 0x1000);
