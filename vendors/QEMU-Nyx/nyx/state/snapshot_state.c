...
void serialize_state(const char *filename_prefix, bool is_pre_snapshot)
{
    ...
    if (is_pre_snapshot) {
        ...
        root_snapshot.cap_http_param_vaddr = nyx_global_state->cap_http_param_vaddr;
        root_snapshot.cap_execution_path_vaddr = nyx_global_state->cap_execution_path_vaddr;
...
void deserialize_state(const char *filename_prefix)
{
    ...
    if (header.type == NYX_SERIALIZED_TYPE_PRE_SNAPSHOT) {
        ...
        nyx_global_state->cap_compile_time_tracing_buffer_vaddr =
            root_snapshot.cap_compile_time_tracing_buffer_vaddr;
        nyx_global_state->cap_http_param_vaddr = root_snapshot.cap_http_param_vaddr;
        nyx_global_state->cap_execution_path_vaddr = root_snapshot.cap_execution_path_vaddr;
        nyx_global_state->cap_ijon_tracing_buffer_vaddr =
            root_snapshot.cap_ijon_tracing_buffer_vaddr;
        nyx_global_state->cap_coverage_bitmap_size =
            root_snapshot.cap_coverage_bitmap_size;
...
void serialize_root_snapshot_meta_data(const char *snapshot_dir){
    ...
    assert(yaml_write_uint64_x(fp, "cap_http_param_vaddr", nyx_global_state->cap_http_param_vaddr));
    assert(yaml_write_uint64_x(fp, "cap_execution_path_vaddr", nyx_global_state->cap_execution_path_vaddr));
