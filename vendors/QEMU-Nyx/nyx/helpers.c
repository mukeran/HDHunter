...
nyx_coverage_bitmap_copy_t *new_coverage_bitmaps(void)
{
    ...
    bitmaps->http_param = malloc(0x1000);

    bitmaps->execution_path = malloc(0x1000);
...
void coverage_bitmap_reset(void)
{
    ...
    if (GET_GLOBAL_STATE()->shared_http_param_ptr) {
        memset(GET_GLOBAL_STATE()->shared_http_param_ptr, 0x00, 0x1000);
        http_param_t *http_param = (http_param_t *)GET_GLOBAL_STATE()->shared_http_param_ptr;
        for (int i = 0; i < 10; ++i) {
    ...
    if (GET_GLOBAL_STATE()->shared_execution_path_ptr) {
        memset(GET_GLOBAL_STATE()->shared_execution_path_ptr, 0x00, 0x1000);
    }
}
...
void coverage_bitmap_copy_to_buffer(nyx_coverage_bitmap_copy_t *buffer)
{
    ...
    if (GET_GLOBAL_STATE()->shared_http_param_ptr) {
        memcpy(buffer->http_param, GET_GLOBAL_STATE()->shared_http_param_ptr, 0x1000);
    }
    if (GET_GLOBAL_STATE()->shared_execution_path_ptr) {
        memcpy(buffer->execution_path, GET_GLOBAL_STATE()->shared_execution_path_ptr, 0x1000);
    }
}
...
void coverage_bitmap_copy_from_buffer(nyx_coverage_bitmap_copy_t *buffer)
{
    ...
    if (GET_GLOBAL_STATE()->shared_http_param_ptr) {
        memcpy(GET_GLOBAL_STATE()->shared_http_param_ptr, buffer->http_param, 0x1000);
    }
    if (GET_GLOBAL_STATE()->shared_execution_path_ptr) {
        memcpy(GET_GLOBAL_STATE()->shared_execution_path_ptr, buffer->execution_path, 0x1000);
    }
}
...
bool apply_capabilities(CPUState *cpu)
{
    ...
    if (GET_GLOBAL_STATE()->cap_compile_time_tracing) {
        ...
        assert(remap_slot(GET_GLOBAL_STATE()->cap_http_param_vaddr, 0, cpu, GET_GLOBAL_STATE()->shared_http_param_fd,
                            0x1000, true,
                            GET_GLOBAL_STATE()->cap_cr3));
        set_cap_agent_http_param(GET_GLOBAL_STATE()->auxilary_buffer, true);

        assert(remap_slot(GET_GLOBAL_STATE()->cap_execution_path_vaddr, 0, cpu, GET_GLOBAL_STATE()->shared_execution_path_fd,
                            0x1000, true,
                            GET_GLOBAL_STATE()->cap_cr3));
        set_cap_agent_execution_path(GET_GLOBAL_STATE()->auxilary_buffer, true);
    }
