...
static void nyx_guest_setup_http_param(nyx_interface_state *s,
                                   char                *filename)
{
    void       *ptr;
    int         fd;
    ...
    fd = open(filename, O_CREAT | O_RDWR, S_IRWXU | S_IRWXG | S_IRWXO);
    assert(ftruncate(fd, 0x1000) == 0);
    stat(filename, &st);
    assert(0x1000 == st.st_size);
    ptr = mmap(0, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    GET_GLOBAL_STATE()->shared_http_param_ptr       = (void *)ptr;
    GET_GLOBAL_STATE()->shared_http_param_fd        = fd;
}

static void nyx_guest_setup_execution_path(nyx_interface_state *s,
                                   char                *filename)
{
    void       *ptr;
    int         fd;
    ...
    assert(0x1000 == st.st_size);
    ptr = mmap(0, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    GET_GLOBAL_STATE()->shared_execution_path_ptr       = (void *)ptr;
    GET_GLOBAL_STATE()->shared_execution_path_fd        = fd;
}
...
static bool verify_workdir_state(nyx_interface_state *s, Error **errp)
{
    ...
    assert(asprintf(&tmp, "%s/http_param_%d", workdir, id) != -1);
    if (!file_exits(tmp)) {
        nyx_error("File %s does not exist...\n", tmp);
        free(tmp);
        return false;
    } else {
        nyx_guest_setup_http_param(s, tmp);
    }
    free(tmp);

    assert(asprintf(&tmp, "%s/execution_path_%d", workdir, id) != -1);
    if (!file_exits(tmp)) {
        nyx_error("File %s does not exist...\n", tmp);
        free(tmp);
        return false;
    } else {
        nyx_guest_setup_execution_path(s, tmp);
    }
    free(tmp);
